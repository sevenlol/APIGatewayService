...
public class FindAllAccountSpecification extends AccountSpecification {

	public FindAllAccountSpecification(Account account) {
		super(account);
	...
	List<Account> doActions(MongoTemplate mongoTemplate)
		...
		for(int index = 0; index < userTypeArr.length; index++) {
			...
			AccountSpecification accSpec = AccountSpecificationFactory.getFindAllTypedSpecification(tempAcc);
			try {
